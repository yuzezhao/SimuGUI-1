...
void FancyNavBarPrivate::init()
{
	...
	//by jh
    m_closeAction = new QAction(QIcon("./Icon/tools/close"), tr("Close"), this);
	...
	//by jh
    m_pSideExpandButton->setIcon(QIcon("./Icon/tools/sidebar"));
	...
	//by jh，目前没看到sidebar的用处
	m_pSideExpandButton->setVisible(false);
...
void FancyNavBarPrivate::updateMaximizeButton()
{
    if(m_isMaximized){
		//by jh
        m_minMaxAction->setIcon(QIcon("./Icon/arrow_down"));
        m_minMaxAction->setText(tr("Minimize"));
    }else{
		//by jh
        m_minMaxAction->setIcon(QIcon("./Icon/arrow_up"));
