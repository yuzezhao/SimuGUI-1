...
void MiniStackPrivate::createWindow()
{
	...
	//by jh
    QAction *action = new QAction(QIcon("./Icon/tools/arrow_up"), tr("collapse"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(slotExpand()));
    m_pExpandAction = action;
	//by jh
    action = new QAction(QIcon("./Icon/tools/setup"), tr("setup"), this);
...
void MiniStackPrivate::slotExpand()
{
    ...
    if(m_bExpand){
		//by jh
        m_pExpandAction->setIcon(QIcon("./Icon/tools/arrow_up"));
        m_pExpandAction->setToolTip(tr("collapse"));
        q->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Expanding);
    }else{
		//by jh
        m_pExpandAction->setIcon(QIcon("./Icon/tools/arrow_down"));
